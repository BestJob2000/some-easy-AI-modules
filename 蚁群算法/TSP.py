...
def getdistmat(coordinates):
   num = coordinates.shape[0]    #城市数量
   distmat = np.zeros((num, num))
   for i in range(num):
      for j in range(i, num):
         distmat[i][j] = distmat[j][i] = np.linalg.norm(coordinates[i] - coordinates[j]) #平方和开根号
   return distmat
...
fig.savefig('average_best.png', dpi=300, bbox_inches='tight')
plt.show()
print(lengthbest[-1])
...
plt.savefig('best path.png', dpi=300, bbox_inches='tight')
plt.show()